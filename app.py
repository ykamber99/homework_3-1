...
app.layout = html.Div([
    ...
    #######Duration part######
    html.H4("Select duration length:"),
    html.Div(
        children=[
            html.Div(
                children=[
            ...
            html.Div(
                children=[
                    html.Label('Duration Unit:'),
                    dcc.Dropdown(["S", "D", "W"], "D", id='duration-unit'),
                ],
                style={
                    'display': 'inline-block',
                    'padding-right': '5px'
    ...
    ######Use RTH Part######
    html.H4("Select whether to return data available: "),
    html.Div(
        children=[
            html.Label("Data Available"),
            dcc.RadioItems(
                id="use-rth",
                options=[{'label': "all data", 'value': "0"}, {'label': "only in regular trading hour", 'value': '1'}],
                value='0'
            )
        ]
    ),
...
@app.callback(
    ...
    [State('currency-input', 'value'), State('what-to-show', 'value'),
     ...
     State('duration-length', 'value'), State('duration-unit', 'value'),
...
def update_candlestick_graph(n_clicks, currency_string, what_to_show,
                             ...
                             duration_len, duration_unit, bar_size, use_RTH):
    # n_clicks doesn't
    ...
    if any([i is None for i in [edt_date, edt_hour, edt_minute, edt_second]]):
        ...
        edt_date = ''.join(edt_date.split("-"))
        end_date_time = edt_date + " " + str(edt_hour).zfill(2) + ":" + str(edt_minute).zfill(2) + ":" \
                        + str(edt_second).zfill(2) + " EST"

    duration_str = str(duration_len) + " " + duration_unit
